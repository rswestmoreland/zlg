...
This benchmark is intentionally fail-closed for RSS capture. Any measured row
with a blank max_rss_kb is treated as a benchmark failure.
"""
...
BUILD_PROFILES = SERIOUS_PROFILES + CONTROL_PROFILES
...
def run_probe(args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    with tempfile.TemporaryDirectory(prefix="zlg-phase1e-") as tmp_name:
        tmp = Path(tmp_name)
        ...
        for corpus_name in args.corpora.split(","):
            ...
            if corpus_name != "binaryish":
                ...
                for query_name, query_args in SEARCH_QUERIES:
                    ...
                    for profile in [p for p in SERIOUS_PROFILES if p != baseline_profile]:
                        for stream_decode in [False, True]:
...
def validate_no_blank_rss(rows: list[dict[str, object]]) -> None:
    missing = [row for row in rows if row.get("max_rss_kb") in ("", None)]
    if missing:
        raise SystemExit(f"RSS capture failed for {len(missing)} rows")
...
def write_markdown(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    for corpus in sorted({row["corpus"] for row in compress_rows}):
        ...
        for index, row in enumerate(ranked, 1):
            doc.append(
                f"| {index} | {row['build_profile']} | {row['wall_seconds']} | {row['max_rss_kb']} | "
                ...
                f"{row['build_pushed_edges']} | {row['build_unique_edges']} | {row['build_duplicate_ratio']} | {row['output_bytes']} |"
            )
        doc.append("")
...
def main() -> int:
    ...
    parser.add_argument("--corpora", default="high_dup,high_cardinality,unicode,binaryish")
    parser.add_argument("--output", default="validation_results/phase1e_builder_fairness_bench.md")
    parser.add_argument("--csv", default="validation_results/phase1e_builder_fairness_bench.csv")
    args = parser.parse_args()

    rows = run_probe(args)
    validate_no_blank_rss(rows)
