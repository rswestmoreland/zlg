#!/usr/bin/env python3
"""Phase 1f final builder bakeoff benchmark.

This benchmark narrows builder candidates to the remaining production-facing
profiles and compares zlg compression against gzip build speed, CPU, RSS, and
output size across several corpus shapes. RSS and CPU capture are fail-closed.
"""
...
def run_probe(args: argparse.Namespace) -> list[dict[str, object]]:
    ...
    with tempfile.TemporaryDirectory(prefix="zlg-phase1f-") as tmp_name:
        tmp = Path(tmp_name)
...
def main() -> int:
    ...
    parser.add_argument("--output", default="validation_results/phase1f_final_builder_bakeoff.md")
    parser.add_argument("--csv", default="validation_results/phase1f_final_builder_bakeoff.csv")
