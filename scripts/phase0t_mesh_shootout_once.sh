...
LOG="validation_results/phase0t_mesh_shootout_once.txt"
: > "$LOG"
...
run python3 tools/phase0t_mesh_shootout.py \
  ...
  --output validation_results/phase0t_mesh_shootout.md \
  --csv validation_results/phase0t_mesh_shootout.csv

run bash scripts/phase0k_csv_commit_guard.sh validation_results/phase0t_mesh_shootout.csv
run bash scripts/phase0i_artifact_hygiene_check.sh

echo "phase0t mesh shootout once: pass" | tee -a "$LOG"
