...
TARGET_DIR="target/release-dry-run"
PKG="zlg-${VERSION}-linux-x86_64"
PKG_DIR="${TARGET_DIR}/${PKG}"

cargo build --release
rm -rf "$TARGET_DIR"
mkdir -p "$PKG_DIR/docs/man" "$PKG_DIR/docs/assets"

cp target/release/zlg "$PKG_DIR/zlg"
cp README.md LICENSE LICENSE-MIT LICENSE-APACHE "$PKG_DIR/"
cp docs/COMMAND_REFERENCE.md docs/FORMAT.md docs/BENCHMARKS.md docs/INSTALL.md docs/RELEASE_CHECKLIST.md docs/ROADMAP.md "$PKG_DIR/docs/"
cp docs/assets/*.svg "$PKG_DIR/docs/assets/"
