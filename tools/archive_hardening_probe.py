...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run archive hardening checks")
...
def make_valid_archive(zlg: list[str], work: Path, mode: str) -> tuple[Path, Path]:
    ...
    with raw.open("wb") as fh:
        for i in range(50_000):
            if i == 40_000:
                fh.write(b"ts=2026-05-17 level=error needle=archive_hardening status=failed\n")
            else:
                fh.write(f"ts=2026-05-17 level=info seq={i} status=ok user=user{i % 17}\n".encode())
    result = run(zlg + ["compress", str(raw), str(archive), "--mode", mode])
...
def write_reports(rows: list[dict[str, str]], out_dir: Path) -> None:
    ...
    csv_path = out_dir / "archive_hardening.csv"
    md_path = out_dir / "archive_hardening.md"
    ...
    with md_path.open("w") as fh:
        fh.write("# Archive Hardening Probe\n\n")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zlg_archive_hardening_") as tmp:
        work = Path(tmp)
    ...
    print(f"archive hardening rows={len(rows)} failures={failures}")
