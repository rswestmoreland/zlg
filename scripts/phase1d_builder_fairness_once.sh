...
LOG="validation_results/phase1d_builder_fairness_once.txt"
: > "$LOG"
...
run python3 tools/phase1d_builder_fairness_bench.py \
...
run bash scripts/phase0k_csv_commit_guard.sh validation_results/phase1d_builder_fairness_bench.csv
run bash scripts/phase0i_artifact_hygiene_check.sh

echo "phase1d builder fairness once: pass" | tee -a "$LOG"
