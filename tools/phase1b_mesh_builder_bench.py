#!/usr/bin/env python3
"""Phase 1b mesh summary builder shootout for the narrowed zlg stack.

The on-disk format remains unchanged. This benchmark keeps fixed-lines8192 +
mesh-bigram and compares build profiles that differ only in implementation.
Phase 1b focuses on summary builder variants now that combined won Phase 1a.
"""
...
BUILD_PROFILES = ["current", "combined", "combined-radix", "combined-hash"]
...
def probe(args: argparse.Namespace) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    with tempfile.TemporaryDirectory(prefix="zlg-phase1b-") as tmp_name:
        tmp = Path(tmp_name)
...
def main() -> int:
    ...
    parser.add_argument("--output", default="validation_results/phase1b_mesh_builder_bench.md")
    parser.add_argument("--csv", default="validation_results/phase1b_mesh_builder_bench.csv")
