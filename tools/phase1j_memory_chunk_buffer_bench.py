...
This pass keeps the selected production stack centered on fixed-lines8192,
mesh-bigram ZBM1 v2, combined-bitset-seen, and zstd. It compares zstd levels
against gzip levels and adds diagnostics for chunk buffering, capped 8192-line
chunks, and a stream-per-chunk zstd path.
"""
...
def write_report(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    lines = []
    lines.append("# Phase 1j memory and chunk-buffer diagnostic")
    lines.append("")
    lines.append("Diagnostic-only pass for chunk buffering, zstd level, and gzip level comparison.")
    lines.append("")
    ...
    lines.append("")
    lines.append("| corpus | mode | policy | level | profile | summary | wall_s | cpu_s | rss_kb | output_bytes | max_chunk_bytes | delta_gzip6 | delta_gzip9 | roundtrip |")
    lines.append("|---|---|---|---:|---|---|---:|---:|---:|---:|---:|---:|---:|---|")
    for r in zlg_rows:
        lines.append(
            ...
            f"{r['delta_vs_gzip_9_bytes']} | {r['roundtrip_ok']} |"
        )
    lines.append("")
    lines.append("## Best size per corpus among locked/capped zlg rows")
    lines.append("")
    lines.append("| corpus | best_mode | level | policy | output_bytes | gzip6 | gzip9 |")
    lines.append("|---|---|---:|---|---:|---:|---:|")
    for corpus in by_corpus:
        candidates = [r for r in zlg_rows if r["corpus"] == corpus and r["summary_mode"] == "mesh-bigram"]
        best = min(candidates, key=lambda r: int(r["output_bytes"]))
        lines.append(
            f"| {corpus} | {best['mode']} | {best['zstd_level']} | {best['chunk_policy']} | "
            f"{best['output_bytes']} | {best['gzip_6_bytes']} | {best['gzip_9_bytes']} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def run(args: argparse.Namespace) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="zlg-phase1j-") as tmp_name:
        tmp = Path(tmp_name)
        for corpus in corpora:
            ...
            zlg_configs = []
            for level in (3, 6, 9):
                zlg_configs.append((f"locked-l{level}", "fixed-lines8192", level, "combined-bitset-seen", "mesh-bigram"))
            for level in (6, 9):
                zlg_configs.append((f"stream-zstd-l{level}", "fixed-lines8192", level, "combined-bitset-seen-stream-zstd", "mesh-bigram"))
                zlg_configs.append((f"no-summary-l{level}", "fixed-lines8192", level, "combined-bitset-seen", "none"))
            for cap in ("fixed-lines8192-cap4m", "fixed-lines8192-cap8m", "fixed-lines8192-cap16m"):
                for level in (6, 9):
                    zlg_configs.append((f"capped-{cap}-l{level}", cap, level, "combined-bitset-seen", "mesh-bigram"))
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=REPO / "validation_results/phase1j_memory_chunk_buffer.md")
    parser.add_argument("--csv", type=Path, default=REPO / "validation_results/phase1j_memory_chunk_buffer.csv")
    parser.add_argument("--chunks-csv", type=Path, default=REPO / "validation_results/phase1j_memory_chunk_buffer_chunks.csv")
