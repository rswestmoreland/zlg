#!/usr/bin/env python3
"""Phase 1a build-speed shootout for the narrowed zlg stack.

The runtime format remains unchanged. This benchmark compares implementation
profiles that build the same fixed-lines8192 + mesh-bigram file:

- current: existing SearchSummary::from_bigram_mesh + zstd::stream::encode_all
- mesh-scratch: reusable mesh buffers + zstd::stream::encode_all
- zstd-bulk: existing mesh build + reusable zstd::bulk::Compressor
- combined: reusable mesh buffers + reusable zstd::bulk::Compressor
"""
...
BUILD_PROFILES = ["current", "mesh-scratch", "zstd-bulk", "combined"]
...
def probe(args: argparse.Namespace) -> tuple[list[dict[str, object]], dict[str, object]]:
    ...
    with tempfile.TemporaryDirectory(prefix="zlg-phase1a-") as tmp_name:
        tmp = Path(tmp_name)
...
def write_markdown(path: Path, rows: list[dict[str, object]], meta: dict[str, object]) -> None:
    ...
    for row in rows_for(rows, "compress"):
        doc.append(
            ...
            f"{row['build_total_ns']} | {row['zlg_summary_bytes']} | {row['zlg_compressed_payload_bytes']} |"
        )
...
def main() -> int:
    ...
    parser.add_argument("--output", default="validation_results/phase1a_build_speed_bench.md")
    parser.add_argument("--csv", default="validation_results/phase1a_build_speed_bench.csv")
