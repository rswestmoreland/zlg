#!/usr/bin/env python3
"""Performance smoke bench with zlg fast and standard modes.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run zlg fast/standard performance smoke bench")
    parser.add_argument("--zlg-bin", default=os.environ.get("ZLG_BIN", "target/release/zlg"))
    parser.add_argument("--out-csv", default="validation_results/perf_modes_head_tail_smoke.csv")
    parser.add_argument("--out-md", default="validation_results/perf_modes_head_tail_smoke.md")
...
def add_build_rows(rows: list[dict[str, object]], scenario: str, paths: dict[str, Path], work: Path, zlg_bin: str) -> None:
    ...
    for mode in ("fast", "standard"):
        ...
        zlg_cmd = [zlg_bin, "compress", "--mode", mode, str(plain), str(zlg_path)]
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="zlg-perf-") as temp_dir:
        work = Path(temp_dir)
...
if __name__ == "__main__":
    if os.name != "posix" or not hasattr(os, "wait4"):
        raise SystemExit("performance bench requires POSIX os.wait4 resource accounting")
    try:
