#!/usr/bin/env python3
"""Phase 2i repeated-median wrapper for the Phase 2e smoke bench.

This wrapper intentionally reuses the Phase 2e benchmark so the command
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Phase 2i repeated median zlg bench")
    parser.add_argument("--zlg-bin", default=os.environ.get("ZLG_BIN", "target/release/zlg"))
    parser.add_argument("--out-csv", default="validation_results/phase2i_repeated_median_smoke.csv")
    parser.add_argument("--out-md", default="validation_results/phase2i_repeated_median_smoke.md")
...
def main() -> int:
    ...
    script = Path("tools/phase2e_perf_modes_head_tail_bench.py")
    if not script.exists():
        raise SystemExit(f"missing benchmark script: {script}")

    all_rows: list[dict[str, str]] = []
    with tempfile.TemporaryDirectory(prefix="zlg-phase2i-median-") as temp_dir:
        temp = Path(temp_dir)
